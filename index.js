...
const completed = document.querySelector(".completed__todos");
const ongoing = document.querySelector(".ongoing__todos");
...
todoForm.addEventListener("submit", (e) => {
	...
	createTodoObject(todoInputValue);
	displayTodos();
});
...
function displayTodos() {
	...
	if (todosList.length == 0) {
		...
		`;
		ongoingTodos.appendChild(emptyTodo);
	} else {
		addTodo();
	}
}
...
function addTodo() {
	...
	todos.forEach((todo) => {
		if (todo.status === "ongoing") {
			...
			todoListItem.setAttribute("aria-label", todo.status);
